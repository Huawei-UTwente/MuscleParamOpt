...
function optimizationResultAnalysis_MPO_func(homeDataPath, trialNamesOpt, subj, ...
    ...
    T = length(trialNamesOpt);  % number of data trails involved in the optimization, so far each trail should have the same number of nodes
    ...
    for W5 = W5_list
        ...
        save_path = sprintf('%s/Subj%02d/W5_%02d_W6_%02d', homeSavingPath, subj, W5, W6);
            ...
            for j = 1:J
                ...
                for t = 1:T
                    plot(1:N(t), torque(sum(N(1:t-1))+1:sum(N(1:t)), :),...
                        'k-', 'linewidth', 2.5)
                    hold on
                    plot(1:N(t), res.mom_res(sum(N(1:t-1))+1:sum(N(1:t)), :),...
