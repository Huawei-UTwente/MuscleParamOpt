...
function [df_dx, df_ddx, df_dx_em, df_dp] = ...
    directCollocationDyn_diff_MPO2(x, dx, x_em, lmt, par, M, P, S)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     d_lce = dx(2*M + 1 : 3*M);  % length of contract element differentiation at current frame
    ...
    [~, dfa_da, dfa_dda, dfa_ds_em] ...
        = activationDyn_Groote_diff(a, da, s_em, M);
    
    dfa_dx = zeros(M, S*M);
    dfa_dx(:, 1:M) = diag(dfa_da);
    dfa_dx(:, M + 1:2*M) = diag(dfa_dda);
    dfa_dx_em(:, 4*M + 1:5*M) = diag(dfa_ds_em);
    ...
    df_dx = [dfd_dx; dfa_dx; dfm_dx];
    df_ddx = [dfd_ddx; zeros(M, S*M); zeros(M, S*M)];
    df_dx_em = [zeros(2*M, S*M); dfa_dx_em; zeros(M, S*M)];
    df_dp = [zeros(2*M, P); zeros(M, P); dfm_dp];
