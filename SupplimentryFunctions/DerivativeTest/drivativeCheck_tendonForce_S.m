...
[~, dFse_dlce, dFse_dlce_opt, dFse_dlt_slack, dFse_dtheta0]...
             = tendenForce_Groote_S_diff(lmt, lce, lce_opt, lt_slack, theta0);
...
for ia = 1:length(lce)
   ...
   Fse_up = tendenForce_Groote_S(lmt, lce, lce_opt, lt_slack, theta0);
   
   % get values with upper change
   lce(ia) = lce(ia) - 2*delta_lce;
   Fse_dw = tendenForce_Groote_S(lmt, lce, lce_opt, lt_slack, theta0);
